...
def add_data(letter):
    global filename, df, win, results
    if filename is None or df is None:
        tkinter.messagebox.showerror('DataFrame Error', 'Error: No DataFrame loaded! Create new file or load an existing one!')
    else:
        # print(results.multi_hand_landmarks[0])
        # exit()
        landmarks = []
        for l in results.multi_hand_landmarks[0].landmark:
            landmarks += [l.x, l.y, l.z]
        for l in results.multi_hand_landmarks[0].landmark:
            landmarks += [l.x, l.y, l.z]
...
def backup_data():
    ...
    if type(df) == pd.DataFrame:
        ...
        tkinter.messagebox.showerror('DataFrame Error',
...
SINGS = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y']
photos = {letter: PhotoImage(file = r"./sign/"+letter+".png") for letter in SINGS}

buttons = [
   Button(button_label, text = letter, image=photos[letter], command=lambda letter=letter: add_data(letter)).grid(row=int(i/6), column=i%6)
   for i, letter in enumerate(SINGS)
]
with mp_hands.Hands(
    model_complexity=0,
